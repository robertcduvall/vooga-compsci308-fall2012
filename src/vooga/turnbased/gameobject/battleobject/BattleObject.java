...
public abstract class BattleObject extends GameObject {
	...
	/**
	 * Create the BattleObject for this sprite which will be used in the
	 * BattleMode.
	 * 
	 * @param allowableModes
	 *            A set of Strings that holds all the possible modes this can
	 *            exist in
	 * @param condition
	 *            The action that this object can pass to the GameManager, can
	 *            be GameEvent.NO_ACTION if no action needed
	 * @param stats
	 *            the battle stats of the object, including its defense, attack,
	 *            and health
	 * @param name
	 *            the name of the object
	 * @param image
	 *            The image of this BattleObject
	 */
	public BattleObject(Set<String> allowableModes, String condition,
	...
	/**
	 * Initializes the stats of the BattleObjects to their default values (i.e.
	 * before each battle)
	 */
	 ...
	 * 
	 * @param stats
	 *            A map of Attributes (Strings) to values (Numbers)
	 */
	 ...
	 * 
	 * @param statName
	 *            The stat you'd like to change
	 * @param value
	 *            The value you want to change the stat to
	 */
	...
	/**
	 * increase a particular stat by given amount
	 * 
	 * @param statName
	 *            the stat name
	 * @param value
	 *            the amount of increase (can be negative)
	 */
	public void increaseStat(String statName, Number value) {
		changeStat(statName,
				getStat(statName).doubleValue() + value.doubleValue());
	}
	 ...
	 * 
	 * @param target
	 *            The target enemy of this object, should it choose to attack
	 *            it.
	 * @return Returns the message that describes what was the BattleObject just
	 *         did
	 */
	 ...
	 *            The int of the Option that was selected
	 * @param target
	 *            The target of the option
	 * @param battleMessages
	 * @return Returns a message that describes what the Action was that the
	 *         BattleObject just took.
	 */
	public abstract void doOption(int MenuOptionSelected, BattleObject target,
			List<String> battleMessages);
	 ...
	 * 
	 * @param target
	 *            The target of this move/attack, can be null, depending on
	 *            implementation.
	 * @return Returns a message that describes what the Action was that the
	 *         BattleObject just took.
	 */
	protected abstract void doOption1(BattleObject target,
			List<String> battleMessages);
	 ...
	 *         BattleObject just took.
	 */
	protected abstract void doOption2(BattleObject target,
			List<String> battleMessages);
	 ...
	 *         BattleObject just took.
	 */
	protected abstract void doOption3(BattleObject target,
			List<String> battleMessages);
	 ...
	 *         BattleObject just took.
	 */
	protected abstract void doOption4(BattleObject target,
			List<String> battleMessages);

	/**
	 * Implement this method to determine how much of the attack done to this
	 * monster actually affects its health.
	 * 
	 * @param damageDone
	 *            The raw damage done by the other monster's attack.
	 */
	public abstract void takeDamage(int damageDone, List<String> battleMessages);

	/**
	 * Implement this method to determine how to attack the other enemy,
	 * depending on which stats for input. Can call
	 * <enemy>.takeDamage(damageDone) once the total amount of damage is called.
	 * 
	 * @param enemy
	 *            The other BattleObject which this BattleObject is fighting.
	 */
	 ...
	 *            Pass True if this is a player controlled BattleObject, False
	 *            if it is not
	 * @return The String the will be displayed when this monster starts
	 *         fighting
	 */
	public abstract String getStartFightingMessage(boolean isPlayerControlled);

	/**
	 * Gets the options that this BattleObject can perform, to be displayed in
	 * the GUI
	 * 
	 * @return String array of the options, paradigm is defaulted to 4, could be
	 *         extended by a game developer
	 */
	...
	public void paintBattleObject(Graphics g, int x, int y, int width,
			int height) {
	...
	/**
	 * Paints the current stats for this BattleObject. Will be drawn at x,y,
	 * with given width and height.
	 * 
	 * @param g
	 *            This is passed in to draw the stats.
	 * @param x
	 *            The x location of where the stats should be drawn
	 * @param y
	 *            The y location of where the stats should be drawn
	 * @param width
	 *            The width of the box for the stats
	 * @param height
	 *            The height of the box for the stats.
	 */
	public void paintStats(Graphics g, int x, int y, int width, int height) {
		...
		g2d.drawGlyphVector(gv, x + STAT_FONT_SHIFT, y + fontSize
				* FONT_SPACING_SCALAR);
		...
		for (int i = 0; i < statLines.length; i += 1) {
			...
			g2d.drawGlyphVector(gv, x + STAT_FONT_SHIFT, y + (2 + i) * fontSize
					* FONT_SPACING_SCALAR);
		}
	}
	 ...
	 * 
	 * @param imgLoop
	 *            The ImageLoop that will be used for animation
	 */
