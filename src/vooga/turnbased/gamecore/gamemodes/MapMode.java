...
public class MapMode extends GameMode implements InputAPI {
	...
	private void updateCameraPosition() {
		...
		if (myTopLeftCoord.x * myCurrentTileWidth + myPlayer.getDirection().x < 0) {
			...
			displacement.x = 0;
				...
				+ myPlayer.getDirection().x > myMapSize.width
				* myCurrentTileWidth) {
		...
		if (myTopLeftCoord.y * myCurrentTileHeight + myPlayer.getDirection().y < 0) {
			...
			displacement.y = 0;
				...
				+ myPlayer.getDirection().y > myMapSize.height
				* myCurrentTileHeight) {
	...
	public void handleMouseClicked(MouseEvent e) {
		...
		if ((e.getModifiers() & MouseEvent.BUTTON3_MASK) != 0) {
			...
			Point target = new Point(e.getX() / myCurrentTileWidth
					...
					+ myTopLeftCoord.y);
			myPathFinder = new MapModePathFinder(this, myPlayer, target,
					myMapSize);
		}
	}
	...
	public void configureInputHandling() {
		try {
			...
			GamePane.keyboardController.setControl(KeyEvent.VK_UP,
					KeyboardController.PRESSED, myPlayer, "moveUp");
			GamePane.keyboardController.setControl(KeyEvent.VK_RIGHT,
					KeyboardController.PRESSED, myPlayer, "moveRight");
			GamePane.keyboardController.setControl(KeyEvent.VK_DOWN,
					KeyboardController.PRESSED, myPlayer, "moveDown");
			GamePane.keyboardController.setControl(KeyEvent.VK_R,
					KeyboardController.PRESSED, myPlayer, "toggleRunning");
			// enable/disable multi-destination feature in PathFinder
			GamePane.keyboardController.setControl(KeyEvent.VK_SHIFT,
					KeyboardController.PRESSED, myPathFinder,
					"activateMultiDestination");
			GamePane.keyboardController.setControl(KeyEvent.VK_SHIFT,
					KeyboardController.RELEASED, myPathFinder,
					"deactivateMultiDestination");
	...
	public boolean isWithinBounds(Point dest) {
		return myMapSize.height > dest.y && myMapSize.width > dest.x
				&& dest.x >= 0 && dest.y >= 0;
	}

	@Override
	public void processMouseInput(int mousePressed, Point myMousePosition,
			int myMouseButton) {
		if (mousePressed == GamePane.MOUSE_CLICKED
				&& myMouseButton == MouseEvent.BUTTON3) {
			myPathFinder.stop();
			Point target = new Point((int) myMousePosition.getX()
					/ myCurrentTileWidth + myTopLeftCoord.x,
					(int) myMousePosition.getY() / myCurrentTileHeight
							+ myTopLeftCoord.y);
			myPathFinder = new MapModePathFinder(this, myPlayer, target,
					myMapSize);
