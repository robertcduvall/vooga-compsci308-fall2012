...
public class WallBuilder {
    ...
    public void buildWall (LineSegment l) {
        ...
        for (int i = 0; i < numSquares; i++) {
            if (i % 5 == 0) {
                mySound.playSound(SoundManager.PLANT_SQUARE_SOUND);
            }
    ...
    public void collide (Square collider) {
        for (int i = myWallSegments.size() - 1; i >= 0; i--) {
            ...
            if (s.intersects(collider)) {
                s.kill();
                collider.addExternalForce(new ExternalMathVector2D((MathVector2D) collider
                                                                   .getLastMovementVector().scale(-2), .8));

                mySound.playSound(SoundManager.WALL_BALL_BREAK);
