    ...
    public void checkRule () {
        for (Square attacker : myAttackers) {
            if (attacker.intersects(myDefender)) {
                myDefender.damage();
                myDefender.addExternalForce(new ExternalMathVector2D((MathVector2D) myDefender
                        .getLastMovementVector().scale(-3.5), .8));
                attacker.addExternalForce(new ExternalMathVector2D((MathVector2D) attacker
                        .getLastMovementVector().scale(-5.5), .8));
